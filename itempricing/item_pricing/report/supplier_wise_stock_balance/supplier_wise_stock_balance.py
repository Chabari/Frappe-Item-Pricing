...
def get_items(filters):
	...
	if filters.get("item_code"):
		...
		if filters.get("supplier"):
			cf_join = "inner join `tabItem Default` itmdef on itmdef.parent=item.name"
			
			conditions.append("itmdef.default_supplier=%(supplier)s")

	items = []
	if conditions:
		# items = frappe.db.sql_list(
		# 	"""select item.name from `tabItem` item  where {}""".format(" and ".join(conditions)), filters
		# )
		items = frappe.db.sql_list(
			"""select item.name from `tabItem` item %s  where %s"""
   		% (cf_join, " and ".join(conditions)),
     		filters
		)
	else:
		items = frappe.db.sql_list(
			"""select item.name from `tabItem` item where item.disabled=0""")
	return items
...
def get_item_details(items, sle, filters):
	...
	res = frappe.db.sql(
		"""
		select
			item.name, item.item_name, itmdef.default_supplier, item.description, item.item_group, item.brand, item.stock_uom %s
		from
			`tabItem` item
		left join `tabItem Default` itmdef on itmdef.parent = item.name
			%s
		where
			item.name in (%s)
	"""
